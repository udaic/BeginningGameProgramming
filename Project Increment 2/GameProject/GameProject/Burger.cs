    ...
    public class Burger
    {
        ...
        public void Update(GameTime gameTime, MouseState mouse)
        {
            ...
            if (health > 0)
            {
                ...
                if(mouse.LeftButton == ButtonState.Pressed && canShoot.Equals(true))
                {

                    canShoot = false;
                    // creation of a french fry proectile happens here

                    // parameters and variable added
                    ProjectileType type = ProjectileType.FrenchFries;
                    Texture2D frySprite = Game1.GetProjectileSprite(type);
                    int x = drawRectangle.Center.X;
                    int y = drawRectangle.Center.Y + GameConstants.FrenchFriesProjectileOffset;

                    // projectile called and added
                    Projectile projectile = new 
                        Projectile(type, frySprite, x, y, GameConstants.FrenchFriesProjectileSpeed);
